...
/*
// function for calculate the number of cookie in the game 

function totalCookie () {
    cookieDisplay.innerHTML = `Nombre de Coco cliker ${Math.floor(cookies)}`;
    cokkie ++; ???? good ???


};

//function calculate total of panier (muliplier) use in the game !

function totalMultiplier () {
    multiplierBtn.innerHTML = `Nombre de Panier acheté: ${Math.floor(multiplier.amount)}X <br> Price: ${Math.floor(multiplier.price)}`

};



//function calculate total of palmier (autocliker) use in the game !

function totalAutoclicker() {
    automaterBtn.innerHTML = `Nombre de Palmier acheté: ${Math.floor(automater.amount)}<br> Prix: ${Math.floor(automater.price)}`;
    
};



//function calculate total of singe (boost) use in the game !

function totalBoost () {
    boostBtn.innerHTML = `Nombre de Singe acheté (200% booster) <br> Price: ${Math.floor(boosters.price)}`;
